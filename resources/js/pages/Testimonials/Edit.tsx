...
import { ArrowLeft, Save, Trash2, User, Star } from 'lucide-react';
import type { Testimonial, Project } from '@/types';
import { useState } from 'react';
import { useForm, router } from '@inertiajs/react';
import * as testimonialsRoutes from '@/routes/testimonials';

type TestimonialsEditProps = {
    testimonial: Testimonial;
    projects: Project[];
};

export default function Edit({ testimonial, projects }: TestimonialsEditProps) {
    const { data, setData, post, processing, errors } = useForm({
        _method: 'PUT' as const,
        project_id: testimonial.project_id?.toString() ?? '',
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        post(testimonialsRoutes.update.url(testimonial.id));
    }
    ...
    return (
        <AppLayout>
            <Head title={`Editar: ${testimonial.client_name}`} />
                            ...
                            <div>
                                ...
                                <div className="mt-2 flex items-center gap-4">
                                    ...
                                    <div className="flex-1 space-y-2">
                                        ...
                                        {currentAvatar && !data.remove_avatar && (
                                            <Button
                                                type="button"
                                                variant="ghost"
                                                size="sm"
